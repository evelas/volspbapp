...
function login() {
    ...
    try {
        ...
        $db = getDB();
        $userData ='';
        $sql = "SELECT user_id, name, email, lastname, gender, admin, score FROM mobileusers WHERE email=:email and password=:password ";
        $stmt = $db->prepare($sql);
        ...
        $db = null;
         if($userData){
               ...
               echo '{"error":{"text":"Bad request wrong email or password"}}';
            }
...
function signup() {
    ...
    try {
        ...
        if (strlen(trim($password))>0 && strlen(trim($email))>0 && $email_check>0 && $password_check>0)
        {
            ...
            $db = getDB();
            $userData = '';
            $sql = "SELECT user_id FROM mobileusers WHERE email=:email";
            $stmt = $db->prepare($sql);
            ...
            if($mainCount==0)
            {
                ...
                $sql1="INSERT INTO mobileusers(lastname,password,email,name,gender,admin,score)VALUES(:lastname,:password,:email,:name,:gender,:admin,:score)";
                ...
                $stmt1->bindParam("admin", $admin,PDO::PARAM_STR);
...
function internalUserDetails($input) {
    ...
    try {
        $db = getDB();
        $sql = "SELECT user_id, name, email, lastname, gender, admin, score FROM mobileusers WHERE email=:input";
        $stmt = $db->prepare($sql);
        $stmt->bindParam("input", $input,PDO::PARAM_STR);
        $stmt->execute();
        $emailDetails = $stmt->fetch(PDO::FETCH_OBJ);
        $emailDetails->token = apiToken($emailDetails->user_id);
        $db = null;
        return $emailDetails;
...
//     $systemtoken=apiToken(used_id);

//     try{
//         if($token == $systemtoken){
//         $db = getDB();
//         $sql = "SELECT * FROM feed WHERE used_id_fk=:used_id";
//         $stmt = $db->prepare($sql);
//         $stmt->bindParam("used_id", $used_id,PDO::PARAM_INT);
//         $stmt->execute();
//         $feedData = $stmt->fetcAll(PDO::FETCH_OBJ) 
//         $db =  null;
